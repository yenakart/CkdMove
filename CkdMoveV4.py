...
def load_config():
    ...
    try:
        ...
        LOGGING_ENABLED = config['settings'].getboolean('enable_logging', fallback=True)
...
class FileMoverApp:
    ...
    def __init__(self, root, source_dir, target_dir, interval, max_rows):
        self.root = root
        self.root.title("File Mover GUI V0.4")
    ...
    def move_files_loop(self):
        while self.is_running:
            try:
                ...
                if LOGGING_ENABLED:
                    logging.exception("Unexpected error in move_files_loop")
    ...
    def move_files(self):
        if not os.path.isdir(self.source_dir):
            ...
            self.root.after(0, lambda: self.status_label.config(text="Source not accessible."))
            return
...
if __name__ == "__main__":
    ...
    if LOGGING_ENABLED:
        ...
        logging.info("=== FileMoverApp Started ===")
